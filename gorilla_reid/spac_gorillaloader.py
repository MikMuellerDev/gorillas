...
class GorillaTripletDataPoint():
    ...
    def to_tensor(self) -> GorillaTripletTensor:
        ...
        return GorillaTripletTensor(
            this_class=rimage(self.this_class.filepath),
            in_class=rimage(self.in_class.filepath),
            out_class=rimage(self.out_class.filepath),
        )
...
class GorillaDataset(Dataset):
    def __init__(
        ...
        # Delete all gorillas that have less than N 
        ...
        print(f"Deleted {len_before - len(self.datapoints)} datapoints due to data distribution.")
