...
def visualize_embeddings(embeddings, labels=None, method='tsne', figsize=(10, 6), 
    ...
    labels_int = [labels_map[label] for label in labels]
    ...
    if method == 'both':
        ...
        # Plot t-SNE
        scatter1 = axes[0].scatter(emb_2d_tsne[:, 0], emb_2d_tsne[:, 1], c=labels_int, cmap="hsv", alpha=0.6, s=20)
        axes[0].set_title(f't-SNE Projection\n{title}')
        axes[0].set_xlabel('Component 1')
        axes[0].set_ylabel('Component 2')

        # Plot PCA
        scatter2 = axes[1].scatter(emb_2d_pca[:, 0], emb_2d_pca[:, 1], c=labels_int, cmap="hsv", alpha=0.6, s=20)
        ...
        scatter = ax.scatter(emb_2d[:, 0], emb_2d[:, 1], c=labels_int, cmap="hsv", alpha=0.6, s=20)
    ...
    if labels_int is not None:
        print(f"Number of unique clusters: {len(np.unique(labels_int))}")
