# An opinionated template for reproducible deep learning research code

NLP research environment using PyTorch + Lightning + Weights & Biases + HuggingFace. It's built to be customized but provides comprehensive, sensible default functionality.
...
The preferred method is to install `conda-lock` using `pipx install conda-lock`. For other options, visit the [conda-lock repo](https://github.com/conda/conda-lock). For basic usage, have a look at the commands below. It is already installed in the devcontainer.
...
For fully reproducible environments and running on HPC clusters, we provide pre-built docker images at [maxscha/research-environment](https://hub.docker.com/r/maxscha/research-environment/tags). We also provide a `Dockerfile` that allows you to build new docker images with updated dependencies:
...
bash ./scripts/run-in-docker.sh -g 2 bash
```
